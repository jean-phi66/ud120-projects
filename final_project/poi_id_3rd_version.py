...
## credit to http://scikit-learn.org/stable/auto_examples/model_selection/plot_confusion_matrix.html
...
def print_metrics(predicted, labels_test, graph=False):
    from sklearn.metrics import confusion_matrix
    from sklearn.metrics import precision_score
    from sklearn.metrics import recall_score
    score = accuracy_score(predicted, labels_test)
    ...
    pprint(cm)
    if(graph):
        plot_confusion_matrix(cm, ['non-poi', 'poi'])
    return
...
predicted = clf.predict(features_test)
print_metrics(predicted, labels_test, graph=True)
